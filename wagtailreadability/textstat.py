...
def sentence_count(text):
    '''
    Counts the number of sentences in a chunk of text.
    '''
    ignore_count = 0
    ...
    return len(sentences) - ignore_count
...
def avg_sentence_length(text):
    ...
    try:
        ...
        raise ValueError('Not enough text has been provided to be rated.')
...
def avg_syllables_per_word(text):
    ...
    try:
        return round(syllable / words, 1)
    except ZeroDivisionError:
        raise ValueError('Cannot rate nothing.')


def avg_letter_per_word(text):
    chars = char_count(text)
    lexicon = lexicon_count(text)
    try:
        return round(chars / lexicon, 2)
    except ZeroDivisionError:
        raise ValueError('Cannot rate nothing.')
...
def avg_sentence_per_word(text):
    ...
    lexicons = lexicon_count(text)
    try:
        return round(sentences / lexicons, 2)
    except ZeroDivisionError:
        raise ValueError('Cannot rate nothing.')
...
def smog_index(text):
    sentences = sentence_count(text)
    if sentences >= 3:
        polysyllable = polysyllable_count(text)
        SMOG = (1.043 * (30*(polysyllable/sentences))**.5) + 3.1291
        return round(SMOG, 1)
    else:
        return 0
...
def coleman_liau_index(text):
    ...
    S = round(avg_sentence_per_word(text)*100, 2)
...
def automated_readability_index(text):
    ...
    wrds = lexicon_count(text)
    snts = sentence_count(text)
    try:
        a = chrs / wrds
        b = wrds / snts
    except ZeroDivisionError:
...
def linsear_write_formula(text):
    ...
    for value in text_list:
        syllables = syllable_count(value)
        if syllables < 3:
            easy_words += 1
        elif syllables > 3:
            difficult_words += 1
    number = (easy_words*1 + difficult_words*3)/sentence_count(text)
    if number > 20:
        number /= 2
    else:
        number = (number-2)/2
...
def dale_chall_readability_score(text):
    ...
    easy_count = word_count - difficult_words(text)
    if word_count > 0:
        easy_per = easy_count / word_count * 100
    else:
        raise ValueError('Cannot rate nothing.')
    difficult_per = 100 - easy_per
    if difficult_per > 5:
        score = (0.1579 * difficult_per) + (0.0496 * avg_sentence_length(text)) + 3.6365
    else:
        score = (0.1579 * difficult_per) + (0.0496 * avg_sentence_length(text))
    return round(score, 2)


def gunning_fog(text):
    try:
        per_diff_words = (difficult_words(text)/lexicon_count(text)*100) + 5
...
def text_standard(text):
    ...
    upper = math.ceil(flesch_grade)
    lower = round(flesch_grade)
    ...
    lower = math.floor(smog_index(text))
    ...
    lower = math.floor(dale_chall_readability_score(text))
    upper = math.ceil(dale_chall_readability_score(text))
    grade.append(int(lower))
    grade.append(int(upper))

    #  Appending Linsear_Write_Formula
    lower = math.floor(linsear_write_formula(text))
    upper = math.ceil(linsear_write_formula(text))
    grade.append(int(lower))
    grade.append(int(upper))

    # Appending Gunning Fog Index
    lower = math.floor(gunning_fog(text))
